...
## Project Overview
- **Name**: burglary
- **Purpose**: Process 5 LinkedIn-sourced business requests through deep multi-round Claude analysis. Phone-only user input. Output per case: business deliverable + case study + LinkedIn post. Target: ~20h Claude Max per case (4× 5h windows), ~100h total.
- **Path**: `/home/diabasis/codeman-cases/burglary`
- **Last Updated**: 2026-04-27

## Architecture

This is not a code project. It's a content/analysis pipeline.

- `WORKFLOW.md` — master playbook. 6-phase per-case pipeline with phone trigger phrases.
- `prompts/viewpoints.md` — catalog of 20 analytical lenses + selection guidance.
- `scripts/run_case.sh` — headless launcher. `bash scripts/run_case.sh <slug> {analyze|deliverable|wrap} [--yolo]` shells out to `claude -p --permission-mode auto --effort max`.
- `scripts/prompts/{analyze,deliverable,wrap}.md` — phase prompts with wall-time floors (4h / 2h / 1h).
- `cases/INDEX.md` — case ledger.
- `cases/usage_log.md` — sub-agent / wall-time tracking for case-study compute accounting.
- `cases/<slug>/` — per-case work, numbered `00_intake.md` through `06_linkedin.md`. Headless logs in `cases/<slug>/_logs/`.

Phase 3 (multi-round analysis) is the credit furnace: 8–10 parallel viewpoint sub-agents → main-thread synthesis → 1 red-team agent → 5 deep-dive agents → final synthesis → Round F (red-team v2 + steelman + reviewer) → Phase 4 deliverable + reviewer → Phase 5/6 wrap drafts. ~25–30 sub-agent invocations per case, ~2M tokens, ~30 min wall.

`new case <slug>: <paste>` triggers the FULL pipeline (phases 0–6 + reviewer chain) end-to-end with no further user input. Assumptions are committed automatically to `_logs/decisions.md`; the user can override any with `override <topic>: <value>`.

Always start by reading `WORKFLOW.md`. The "Full pipeline orchestration" section is the canonical sequence.
...
| Date | Tasks Completed | Files Changed | Notes |
|------|-----------------|---------------|-------|
| 2026-04-27 | Project created | CLAUDE.md | Initial setup |
